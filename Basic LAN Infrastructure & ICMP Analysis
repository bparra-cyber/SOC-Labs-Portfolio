...
*   **Hardware Selection:** Deployed two End Devices (PC0 and PC1) and connected them to a Cisco Catalyst 2960 Layer 2 switch. Chosen because it represents standard enterprise hardware rather than a generic simulation device.
...
*   **Configuration:** Configured PC0 with static IP `192.168.1.10` and PC1 with `192.168.1.20`. Both utilize a `/24` subnet mask (`255.255.255.0`), placing them inside the same local network segment (`192.168.1.0`).
*   **Testing Tool:** Opened the command line interface on PC0 and executed a network connectivity test using the standard `ping` utility directed at PC1.
*   **Analysis of Results:** 
